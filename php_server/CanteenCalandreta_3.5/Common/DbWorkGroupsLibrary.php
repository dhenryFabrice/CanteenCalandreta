...
/**
 * Check if a given supporter is a referent of a workgroup, thanks to its ID
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2019-11-22
 ...
 *
 * @return Boolean                       TRUE if the supporter is a referent of a workgroup,
 *                                       FALSE otherwise
 */
 function IsWorkgroupReferent($DbConnection, $SupportMemberID)
 {
     if ($SupportMemberID > 0)
     {
         $DbResult = $DbConnection->query("SELECT wgr.WorkGroupRegistrationID
                                           FROM WorkGroupRegistrations wgr, WorkGroups wg
                                           WHERE wgr.WorkGroupID = wg.WorkGroupID AND wgr.WorkGroupRegistrationReferent = 1
                                           AND wgr.SupportMemberID = $SupportMemberID");
         if (!DB::isError($DbResult))
         {
             if ($DbResult->numRows() > 0)
             {
                 // The supporter is a referent for some workgroups
                 return TRUE;
             }
             else
             {
