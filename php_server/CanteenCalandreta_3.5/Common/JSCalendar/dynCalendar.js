...
*
* Updated by Christophe Javouhey :
*     - 2019-11-08 : taken into account language
*/
...
* @param string OPTIONAL     Optional language
*/
        ...
        */
                ...
                Pos = arguments[2].indexOf("CanteenCalandreta", 0);
                if (Pos == -1)
                {
                ...
                {
                    this.imagesPath = location.protocol + "//" + location.hostname + "/" + arguments[2].substring(Pos, arguments[2].length - 1) + "/";
                }

                this.objInputName = arguments[3] ? arguments[3] : null;

                this.layerID        = arguments[4] ? arguments[4] : 'dynCalendar_layer_' + dynCalendar_layers.length;

                // Set calendar language
                this.language = arguments[5]? arguments[5] : 'en';
...
*/
        function dynCalendar_show()
        {
            ...
            */
                        ...
                        linkHTML = '<a href="javascript:' + this.callbackFunc + '(' + i + ', ' + (Number(month) + 1) + ', ' + year + ", '" + this.objInputName + "'); " + this.objName + '._hideLayer()">' + (i++) + '</a>';
        ...
        */
                ...
                html = '<table border="0" bgcolor="#eeeeee" cellspacing="0">';
                html += '<tr class="header_calendar"><td class="dynCalendar_header">' + prevLinkHTML + '</td><td colspan="5" align="center" class="dynCalendar_header">' + monthCombo + ' ' + yearCombo + '</td><td align="right" class="dynCalendar_header">' + nextLinkHTML + '</td></tr>';
                html += '<tr>';
                ...
                html += '<tr class="days_calendar">' + ret.join('</tr>\n<tr class="days_calendar">') + '</tr>';
                html += '</table>';
...
*/
        function dynCalendar_writeHTML()
        {
                if (is_ie5up || is_nav6up || is_gecko) {
                        if (this.objInputName == null) {
                            document.write('<a href="javascript:' + this.objName + '.show()"><img src="' + this.imagesPath + 'dynCalendar.png" border="0" width="16" height="16" /></a>');
                            ...
                            var objInput = document.getElementById(this.objInputName);
                            if (objInput) {
                                var objLink = document.createElement('a');
                                objLink.setAttribute('href', 'javascript:' + this.objName + '.show()');
                                objLink.innerHTML = '<img src="' + this.imagesPath + 'dynCalendar.png" border="0" width="16" height="16" />';

                                objInput.parentNode.insertBefore(objLink, objInput.nextSibling);

                                var objDivCal = document.createElement('div');
                                objDivCal.setAttribute('id', this.layerID);
                                objDivCal.className = "dynCalendar";
                                objDivCal.setAttribute('onmouseover', this.objName + "._mouseover(true)");
                                objDivCal.setAttribute('onmouseout', this.objName + "._mouseover(false)");

                                objLink.parentNode.insertBefore(objDivCal, objLink.nextSibling);
                            }
                        }
                }
        }
...
*/
        ...
        {
                if (dynCalendar_mouseY - document.body.scrollTop + this.offsetY + 160 > document.body.clientHeight)
                {
                    this._getLayer().style.top  = (dynCalendar_mouseY - this.offsetY - 160) + 'px';
                }
                else
                {
                ...
                if (dynCalendar_mouseX - document.body.scrollLeft + this.offsetX + 210 > document.body.clientWidth)
                {
                    this._getLayer().style.left = (document.body.clientWidth - 210) + 'px';
                }
                else
                {
