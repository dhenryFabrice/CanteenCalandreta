 ...
 * @author Christophe Javouhey
 * @version 2.9
 *     - 2013-04-05 : v2.0. Taken into account events of the Cooperation module
 *     - 2014-02-03 : v2.1. Taken into account nursery delays
 *     - 2015-07-09 : v2.2. Taken into account snack registrations, laundry registrations and exit permissions
 *     - 2015-10-13 : v2.3. Taken into account workgroups of the Cooperation module
 *     - 2016-03-04 : v2.4. Taken into account towns, alias, messages sent and donations
 *     - 2016-10-28 : v2.5. Taken into account SWAP action for snack and laundry plannings
 *     - 2017-10-06 : v2.6. Taken into account discounts of families
 *     - 2019-05-07 : v2.7. Taken into account documents approvals
 *     - 2019-11-20 : v2.8. Taken into account uploaded files linked to objects and meeting rooms registrations
 *     - 2020-02-20 : v2.9. Taken into account $CONF_NURSERY_OTHER_TIMESLOTS
 *
 * @since 2012-01-12
 *
             ...
             )
         {
             ...
             if (in_array($Level, $GLOBALS['CONF_LOG_EVENTS_LEVELS']))
             {
                 ...
                 switch($ItemType)
                 {
                     ...
                     case EVT_UPLOADED_FILE:
                         switch($Service)
                         {
                             case EVT_SERV_UPLOADED_FILE:
                                 $ID = $ItemID;
                                 switch($Action)
                                 {
                                     case EVT_ACT_ADD:
                                     case EVT_ACT_UPDATE:
                                         if (isExistingUploadedFile($DbConnection, $ItemID))
                                         {
                                             $Record = getTableRecordInfos($DbConnection, 'UploadedFiles', $ItemID);
                                             $LinkedObjectID = $Record['ObjectID'];
                                         }
                                         break;

                                     case EVT_ACT_DELETE:
                                         if ((isset($ArrayInfos['UploadedFileDetails'])) && (!empty($ArrayInfos['UploadedFileDetails'])))
                                         {
                                             $Record = $ArrayInfos['UploadedFileDetails'];
                                             $LinkedObjectID = $Record['ObjectID'];
                                         }
                                         break;
                                 }
                                 ...
                                 if (isset($Record))
                                 {
                                     ...
                                     switch($Record['UploadedFileObjectType'])
                                     {
                                         case OBJ_EVENT:
                                             $bObjectExists = isExistingEvent($DbConnection, $Record['ObjectID']);
                                             $RecordLinkedObject = getTableRecordInfos($DbConnection, 'Events', $Record['ObjectID']);
                                             $Name = $RecordLinkedObject['EventTitle'];
                                             $Type = $GLOBALS['LANG_EVENT'];
                                             break;
                                     }
                         ...
                         switch($Service)
                         {
                             case EVT_SERV_PLANNING:
                                 ...
                                 if ((isset($Record)) && (isExistingChild($DbConnection, $Record['ChildID'])))
                                 {
                                     ...
                                     {
                                         $sAmPmOtherTimeslot = $GLOBALS['LANG_AM'];
                                     }
                                     ...
                                     if (!empty($Record['NurseryRegistrationForPM']))
                                     {
                                         ...
                                         $sAmPmOtherTimeslot .= $GLOBALS['LANG_PM'];
                                     }

                                     if (!empty($Record['NurseryRegistrationOtherTimeslots']))
                                     {
                                         // Get other timeslots for the concerned school year
                                         $ConcernedSchoolYear = getSchoolYear($Record['NurseryRegistrationForDate']);
                                         if ((isset($GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$ConcernedSchoolYear]))
                                             && (!empty($GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$ConcernedSchoolYear])))
                                         {
                                             // There are some other timeslots for the concerned school year
                                             // We get which other timeslots are checked
                                             $ArrayOTSPos = array_keys($GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$ConcernedSchoolYear]);
                                             foreach($ArrayOTSPos as $ots => $CurrentOtherTimeslotID)
                                             {
                                                 if ($Record['NurseryRegistrationOtherTimeslots'] & pow(2, $ots))
                                                 {
                                                     ...
                                                     $sAmPmOtherTimeslot .= $GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$ConcernedSchoolYear][$CurrentOtherTimeslotID]['Label'];
                                                 }
                                             }
                                         }
                                     }

                                     $Date.= ", $sAmPmOtherTimeslot";
                         ...
                         switch($Service)
                         {
                             case EVT_SERV_MEETING:
                                 ...
                                 switch($Action)
                                 {
                                     case EVT_ACT_CREATE:
                                     case EVT_ACT_UPDATE:
                                         if (isExistingMeetingRoomRegistration($DbConnection, $ItemID))
                                         {
                                             $Record = getTableRecordInfos($DbConnection, 'MeetingRoomsRegistrations', $ItemID);
                                         }
                                         break;

                                     case EVT_ACT_DELETE:
                                         if ((isset($ArrayInfos['MeetingRoomRegistrationDetails'])) && (!empty($ArrayInfos['MeetingRoomRegistrationDetails'])))
                                         {
                                             $Record = $ArrayInfos['MeetingRoomRegistrationDetails'];
                                         }
                                         break;
                                 }

                                 $Reference = $ItemID;
                                 $Subject = $Record['MeetingRoomRegistrationTitle'];
                                 $Date = date($GLOBALS['CONF_DATE_DISPLAY_FORMAT'], strtotime($Record['MeetingRoomRegistrationStartDate']));

                                 // Get infos about the meeting room
                                 if (isset($Record['MeetingRoomID']))
                                 {
                                     $RecordMeetingRoom = getTableRecordInfos($DbConnection, 'MeetingRooms', $Record['MeetingRoomID']);
                                     $Name = $RecordMeetingRoom['MeetingRoomName'];
                                 }

                                 $Title = $GLOBALS['LANG_MEETING_ROOM_REGISTRATION']." n∞$Reference : $Subject ($Date)";
                                 break;
                         }
                         break;
