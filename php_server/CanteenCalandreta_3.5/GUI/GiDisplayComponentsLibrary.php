 ...
 * @version 2.0
 *     - 2019-11-26 : v2.0. Taken into account the DYN_CSS_STYLE mode
 *
 * @since 2008-09-23
 *
 * @param $ArrayIcons    Mixed array       Contains icons and text about icons
 * @param $Mode          Const             Mode to display the legends (ICON, CSS_STYLE or DYN_CSS_STYLE)
 *
     ...
     if (count($ArrayIcons) > 0)
     {
         switch($Mode)
         {
             case CSS_STYLE:
                 ...
                 {
                     // $CurrentLegent = array(CSS class style, text)
             ...
             case DYN_CSS_STYLE:
                 ...
                 foreach($ArrayIcons as $i => $CurrentLegend)
                 {
                     // $CurrentLegent = array(text, content of the style attribut)
                     $Legend .= "<li><span style=\"".$CurrentLegend[1]."\">".$CurrentLegend[0]."</span></li>";
                 }
                 $Legend .= "</ul>";
                 break;
...
/**
 * Display the upload file form in the current row of the table of the web page, in the graphic interface
 * in XHTML
 *
 * @author Christophe Javouhey
 * @version 1.0
 ...
 * @param $Title                String                Title of the form
 * @param $ProcessFormPage      String                URL of the page which will process the form
 * @param $VerifFct             String                Name of the javascript function (without ()) used to valide the form
 */
 function displayUploadFileForm($Title, $ProcessFormPage, $VerifFct, $ObjectType, $ObjectID)
 {
     if ((isset($_SESSION["SupportMemberID"])) && ($ObjectType > 0) && ($ObjectID > 0))
     {
         // Open a form
         openForm("FormUploadFile", "post", $ProcessFormPage, "multipart/form-data",
                  "$VerifFct('".$GLOBALS["LANG_ERROR_JS_FILENAME"]."', '".implode("#", $GLOBALS['CONF_UPLOAD_ALLOWED_EXTENSIONS'])."', '".$GLOBALS["LANG_ERROR_JS_EXTENSION"]."', '"
                  .$GLOBALS["LANG_UPLOADING"]."')");

         // Display the table (frame) where the form will take place
         openStyledFrame($Title, "UploadFileFrame", "UploadFileFrame", "UploadFileFrame");

         echo "<table cellspacing=\"0\" cellpadding=\"0\">\n<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_FILENAME"]."</td><td class=\"Value\">";
         insertInputField("fFilename", "file", "", "50", $GLOBALS["LANG_FILENAME_TIP"], "");
         insertInputField("MAX_FILE_SIZE", "hidden", "", "", "", $GLOBALS["CONF_UPLOAD_UPLOADED_FILES_MAXSIZE"]);
         echo "</td></tr>\n<tr><td class=\"Label\">".$GLOBALS["LANG_FILE_DESCRIPTION"]."</td><td class=\"Value\">";
         insertInputField("sFileDescription", "text", "255", "63", $GLOBALS["LANG_FILE_DESCRIPTION_TIP"], "");
         echo "</td>\n</tr>\n</table>\n";

         // Display the allowed file extensions
         displayBR(1);
         $Extensions = $GLOBALS["LANG_ALLOWED_FILES_EXTENSIONS"]." : ".implode(", ", $GLOBALS['CONF_UPLOAD_ALLOWED_EXTENSIONS']).".";
         displayStyledText($Extensions, "AllowedExtensions");

         insertInputField("hidObjectType", "hidden", "", "", "", $ObjectType);
         insertInputField("hidObjectID", "hidden", "", "", "", $ObjectID);
         closeStyledFrame();
...
/**
 * Display the list of the uploaded files linked to an object in the current row of the table of the web page,
 ...
 * @since 2019-11-20
 *
 * @param $ObjectType           Integer               Type of concerned object [1..n]
 * @param $ObjectID             Integer               ID of the concerned object [1..n]
 * @param $TabFiles             Array of String       List of the files linked to the given object in parameter
 */
 function displayUploadedFilesOfObject($ObjectType, $ObjectID, $TabFiles)
 {
     if ((!isset($TabFiles["UploadedFileID"])) || (count($TabFiles["UploadedFileID"]) == 0))
     {
         // No file for this object
         displayStyledText($GLOBALS["LANG_NO_FILE_FOR_OBJECT"], "AowFiles");
     }
     else
     {
         switch($ObjectType)
         {
             case OBJ_EVENT:
                 $HTTPDirectory = $GLOBALS["CONF_UPLOAD_EVENTS_FILES_DIRECTORY"];
                 $HDDDirectory = $GLOBALS["CONF_UPLOAD_EVENTS_FILES_DIRECTORY_HDD"];
                 break;
         }
         ...
         echo "<table>\n";
         foreach($TabFiles["UploadedFileID"] as $i => $CurrentValue)
         {
             echo "<tr>\n\t<td>&nbsp;</td><td class=\"AowFiles\">";

             // Get the file size
             $sFileSize = '';
             if (file_exists($HDDDirectory.$TabFiles["UploadedFileName"][$i]))
             {
                 $iFileSize = filesize($HDDDirectory.$TabFiles["UploadedFileName"][$i]);
                 $factor = floor((strlen($iFileSize) - 1) / 3);
                 $sFileSize = sprintf("%.2f", $iFileSize / pow(1024, $factor)).@$sz[$factor];
             }

             // Link to view or download the file
             echo generateCryptedHyperlink($TabFiles["UploadedFileName"][$i], $CurrentValue, $HTTPDirectory.$TabFiles["UploadedFileName"][$i],
                                           $GLOBALS["LANG_VIEW_FILE_TIP"], "AowFiles", "_blank");
             displayStyledText(" - ".date($GLOBALS["CONF_DATE_DISPLAY_FORMAT"].' '.$GLOBALS["CONF_TIME_DISPLAY_FORMAT"],
                                          strtotime($TabFiles["UploadedFileDate"][$i])), "AowFiles");

             if (!empty($sFileSize))
             {
                 displayStyledText(" - $sFileSize ", "AowFiles");
             }

             // Check if the description isn't empty
             if (!empty($TabFiles["UploadedFileDescription"][$i]))
             {
                 displayBR(1);
                 displayStyledText($TabFiles["UploadedFileDescription"][$i], "AowFiles");
             }
             echo "</td><td class=\"AowFormSpace\">&nbsp;</td><td>";

             // Link to delete the file
             if ((isset($TabFiles["CanDelete"])) && ($TabFiles["CanDelete"][$i]))
             {
                 // The file can be deleted
                 echo generateStyledLinkText($GLOBALS["LANG_DELETE"], $DeleteFileScript = "DeleteUploadedFile.php?Type=$ObjectType&amp;Id=$CurrentValue&amp;Cr=".md5($CurrentValue),
                                             'AowFiles', $GLOBALS["LANG_DELETE_FILE_INSTRUCTIONS"], '');
             }
