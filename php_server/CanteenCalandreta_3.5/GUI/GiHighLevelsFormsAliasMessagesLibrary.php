 ...
 * @author Christophe Javouhey
 * @version 1.2
 *     - 2016-10-21 : v1.1. Display checkbox to put in copy the author of a messsage and author
 *                    on read-only access
 *     - 2019-11-19 : v1.2. Display the number of messages not sent
 *
 * @since 2016-03-01
 *
 * @param $DbConnection             DB object             Object of the opened database connection
 ...
 function displaySendMessageForm($DbConnection, $ProcessFormPage, $AccessRules = array(), $FileAccess = FALSE)
 {
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         if (in_array($cUserAccess, array(FCT_ACT_CREATE, FCT_ACT_UPDATE, FCT_ACT_READ_ONLY, FCT_ACT_PARTIAL_READ_ONLY)))
         {
             ...
             // Display the number of messages not sent
             $iNbMessagesNotSent = 0;
             $ArrayMessages = dbSearchJobs($DbConnection, array("JobType" => array(JOB_EMAIL), "JobResult" => 0), "JobID", 1, 0);
             if ((isset($ArrayMessages['JobID'])) && (!empty($ArrayMessages['JobID'])))
             {
                 foreach($ArrayMessages['JobID'] as $j => $CurrentJobID)
                 {
                     foreach($ArrayMessages['JobParameters'][$j] as $p => $CurrentParams)
                     {
                         if ($CurrentParams['JobParameterName'] == 'mailinglist')
                         {
                             // Extract the mailing-list
                             $MailingListTmp = unserialize(base64_decode($CurrentParams['JobParameterValue']));
                             if (!empty($MailingListTmp))
                             {
                                 if (isset($MailingListTmp['to']))
                                 {
                                     $iNbMessagesNotSent += count($MailingListTmp['to']);
                                 }

                                 if (isset($MailingListTmp['cc']))
                                 {
                                     $iNbMessagesNotSent += count($MailingListTmp['cc']);
                                 }

                                 if (isset($MailingListTmp['bcc']))
                                 {
                                     $iNbMessagesNotSent += count($MailingListTmp['bcc']);
                                 }
                             }
                         }
                     }
             ...
             echo $GLOBALS['LANG_MESSAGE_WAITING_MESSAGES']." $iNbMessagesNotSent";
             closeParagraph();
             displayBR(1);
