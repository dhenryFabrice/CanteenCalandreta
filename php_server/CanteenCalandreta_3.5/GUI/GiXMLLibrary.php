 ...
 * @author Christophe Javouhey
 * @version 1.7
 *     - 2012-12-21 : v1.1. Patch a bug about the change of price in a current school year
 *     - 2014-02-03 : v1.2. Taken into account the BillNurseryNbDelays field
 *     - 2014-06-02 : v1.3. Replace an "integer" cast by a round() function and use the abs() function
 *     - 2015-01-16 : v1.4. Try to find the right price of canteen if not found in $CONF_CANTEEN_PRICES
 *                    Do the same treatment to find the right price of nursery
 *     - 2017-11-07 : v1.5. Taken into account BillWithoutMealAmount and the right price
 *     - 2019-07-16 : v1.6. Patch a div by 0 about $fWithoutMealPrice and round() sub-toal and total
 *                    of the bill
 *     - 2020-03-09 : v1.7. Taken into account BillNbCanteenRegistrations and BillNbNurseryRegistrations
 *                          fields
 *
 * @since 2012-02-23
 *
 ...
 function xmlBill($DbConnection, $BillID)
 {
     if (($BillID > 0) && (isExistingBill($DbConnection, $BillID)))
     {
         ...
         if (!empty($RecordBill))
         {
             ...
             if (is_null($RecordBill['BillNbNurseryRegistrations']))
             {
                 // We try to guess the number of nursery registrations. IMPORTANT : work only if only AM and PM nursery timeslots are used
             ...
             {
                 $iNbNurseryRegistrations = $RecordBill['BillNbNurseryRegistrations'];
             }
