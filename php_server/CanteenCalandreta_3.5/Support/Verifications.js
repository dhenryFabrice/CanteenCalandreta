...
/**
 * Function used to check or uncheck all children of a class for a day (a given other timeslot only).
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2020-02-20
 ...
 * @param NumDay             Integer              Number of the day in the month [0..n]
 * @param DateDay            Date                 Date of the day (yyyy-mm-dd)
 * @param OtherTimeslotID    String               ID of the concerned timeslot
 */
 function checkClassNurseryPlanningOtherTimeslot(Class, NumDay, DateDay, OtherTimeslotID)
 {
     var objClassChk = document.getElementById("chkNurseryRegitrationOtherTimeslotClass_" + Class + "_" + NumDay + "_" + OtherTimeslotID);
     for(i = 0 ; i < document.forms[0].length ; i++)
     {
         // Check if it's a field of the class
         if (document.forms[0].elements[i].name == "chkNurseryRegitrationOtherTimeslot" + OtherTimeslotID + "[]")
         {
             // Check if it's a field of the day
             if (document.forms[0].elements[i].value.indexOf(DateDay + "#" + Class + "#", 0) != -1)
             {
                 ...
                 {
                     // Registre the child
                     document.forms[0].elements[i].parentNode.className = "PlanningSupporterFormationOtherTimeslot";
                 }
                 else
                 {
                     // Unregistre the child
                     document.forms[0].elements[i].parentNode.className = "PlanningWorkingDayOtherTimeslot";
                 }
             }
         }
     }
...
/**
 * Function used to set the cell style of a day (given other time slot only) for a child in the nursery planning.
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2020-02-20
 ...
 * @param OtherTimeslotID    String               ID of the concerned timeslot
 */
 function checkChildDayNurseryPlanningOtherTimeslot(Class, NumDay, DateDay, ChildID, OtherTimeslotID)
 {
     for(i = 0 ; i < document.forms[0].length ; i++)
     {
         // Check if it's a field of the class
...
/**
 * Function used to validate the "Upload file" form
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2019-11-20
 *
 * @param FilenameError              String               Error message if no selected file
 * @param AllowedExtensions          Array of Strings     Contains the files extentions allowed to be uploaded
 * @param ExtensionsMsgError         String               Error message if the filename has a not allowed extension
 * @param WaitingMsg                 String               Waiting message displayed during the upload
 *
 * @return Boolean                   TRUE if the "Upload file" form is correctly entered, FALSE otherwise
 */
 function VerificationUploadFile(FilenameError, AllowedExtensions, ExtensionsMsgError, WaitingMsg)
 {
     if ((document.getElementById('fFilename')) && (document.getElementById('fFilename').value != ""))
     {
         if (AllowedExtensions == "")
         ...
         else
         {
             ...
             // We extract the extension of the filename
             var FileExtension = new String(document.getElementById('fFilename').value);
             var iPosExtension = FileExtension.lastIndexOf(".");
...
/**
 * Function used to check/uncheck all families in the "communication" form of an event
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2020-01-22
 */
 function CheckAllEventFamilies()
 {
     var objCheckboxes = document.getElementsByName('chkMsgFamilies[]');
     if (objCheckboxes)
     {
         // Toggle checkboxes
...
/**
 * Function used to validate the meeting room registration form
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2019-11-25
 *
 * @param MeetingRoomMsgError          String    Error message if the meeting room name field is empty
 * @param TitleMsgError                String    Error message if the title field is empty or not valid
 * @param StartDateMsgError            String    Error message if the start date field is empty or not valid
 * @param WrongTimesMsgError           String    Error message if the start eand end times aren't valid
 * @param MailingListMsgError          String    Error message if the mailing field isn't valid
 *
 * @return Boolean                     TRUE if the meeting room registration form is correctly entered, FALSE otherwise
 */
 function VerificationMeetingRoomRegistration(MeetingRoomMsgError, TitleMsgError, StartDateMsgError, WrongTimesMsgError, MailingListMsgError)
 {
     if (document.forms[0].lMeetingRoomID.options[document.forms[0].lMeetingRoomID.selectedIndex].value == 0)
     {
         alert(MeetingRoomMsgError);
         return false;
     }
     else if (document.forms[0].sTitle.value == "")
     {
     ...
     else
     {
         ...
         // Compute the number of days between the start date and the end date
         DeadlineDelay = (StartDate.getTime() - CurrentDate.getTime()) / 86400000;
         if (DeadlineDelay < 0)
         {
             alert(StartDateMsgError);
             return false;
         ...
         // We check if start time < end time
         StartTime = document.forms[0].lStartTime.options[document.forms[0].lStartTime.selectedIndex].value;
         StartDateHM = ListSepToArray(StartTime, ":");
         StartDate = new Date(StartDateDMY[2], StartDateDMY[1] - 1, StartDateDMY[0], StartDateHM[0], StartDateHM[1], 0);

         EndTime = document.forms[0].lEndTime.options[document.forms[0].lEndTime.selectedIndex].value;
         EndDateHM = ListSepToArray(EndTime, ":");
         EndDate = new Date(StartDateDMY[2], StartDateDMY[1] - 1, StartDateDMY[0], EndDateHM[0], EndDateHM[1], 0);
         DeadlineDelay = (EndDate.getTime() - StartDate.getTime()) / 86400000;
         if (DeadlineDelay <= 0)
         {
             alert(WrongTimesMsgError);
             return false;
         }
     }

     if (document.forms[0].sMailingList.value != "")
     {
         // We check if each e-mail of the mailing-list is valid
         var bValid = true;
         ArrayMails = ListSepToArray(document.forms[0].sMailingList.value, ",");
         for(var i = 0; i < ArrayMails.length; i++) {
             if (!isValideEmailAddress(ArrayMails[i].trim())) {
                 bValid = false;
             }
         }
