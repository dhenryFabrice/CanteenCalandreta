...
/**
 * Support module : allow a supporter to create a new suspension for a child.
 * The supporter must be logged to create the suspension.
 *
 ...
 */
 ...
 if (md5($Id) == $CryptedID)
 {
     openParagraph("InfoInstruction");
     displayStyledText($LANG_SUPPORT_ADD_CHILD_SUSPENSION_PAGE_INTRODUCTION, "WarningInstruction");
     closeParagraph();

     displayDetailsChildSuspensionForm($DbCon, 0, $Id, "ProcessNewChildSuspension.php", $CONF_ACCESS_APPL_PAGES[FCT_FAMILY]);
 }
 else
 {
     openFrame($LANG_ERROR);
     displayStyledText($LANG_ERROR_NOT_VIEW_CHILD_SUSPENSION, 'ErrorMsg');
     closeFrame();
 }
