...
/**
 * Support module : process the creation of a new bank. The supporter must be logged.
 *
 ...
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         if ($ContinueProcess)
         {
             $BankID = dbAddBank($DbCon, $BankName);

             if ($BankID != 0)
             {
                 // Log event
                 logEvent($DbCon, EVT_PAYMENT, EVT_SERV_BANK, EVT_ACT_CREATE, $_SESSION['SupportMemberID'], $BankID);

                 // The bank is added
                 $ConfirmationCaption = $LANG_CONFIRMATION;
                 $ConfirmationSentence = $LANG_CONFIRM_BANK_ADDED;
                 $ConfirmationStyle = "ConfirmationMsg";
                 $UrlParameters = "Cr=".md5($BankID)."&Id=$BankID"; // For the redirection
             }
             else
             {
                 // The bank can't be added
                 $ConfirmationCaption = $LANG_ERROR;
                 $ConfirmationSentence = $LANG_ERROR_ADD_BANK;
 ...
 {
     // The supporter doesn't come from the AddBank.php page
     $ConfirmationCaption = $LANG_ERROR;
