 ...
 *     - 2019-11-28 : v3.4. Use dbResetPaymentUsedAmount() to reset the used amount of the payment
 *                          and reset the BillPaidAmount and BillPaid fields
 *
 * @since 2015-01-20
 */
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         if ($ContinueProcess)
         {
             if ((isset($ArrayBills['BillID'])) && (!empty($ArrayBills['BillID'])))
             {
                 ...
                 // We get the FamilyID of the payement
                 $FamilyID = getTableFieldValue($DbCon, 'Payments', $PaymentID, 'FamilyID');
                 ...
                 foreach($ArrayBills['BillID'] as $b => $BillID)
                 {
                     ...
                     {
                         // We check if the bill has other payments linked to it
                         $ArrayBillPayments = getFamilyPayments($DbCon, $FamilyID, array("BillID" => array($BillID)),
                                                                'PaymentDate DESC, PaymentID DESC');
                         if ((empty($ArrayBillPayments))
                             || ((isset($ArrayBillPayments['PaymentID'])) && (empty($ArrayBillPayments['PaymentID']))))
                         {
                             // No payment for this bill : we reset the BillPaidAmount and BillPaid fields
                             $DbCon->query("UPDATE Bills SET BillPaidAmount = 0.00, BillPaid = 0 WHERE BillID = $BillID");
                         }
                     }
                 }
             }
             ...
             if ($bUpdated)
             {
                 // To be sure, we reset the used amount of the payment
                 dbResetPaymentUsedAmount($DbCon, $PaymentID);
