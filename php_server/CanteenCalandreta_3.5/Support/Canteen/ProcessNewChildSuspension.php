...
/**
 * Support module : process the creation of a new suspension for a child. The supporter must be logged.
 *
 ...
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         // We get the ID of the the child concerned by the suspension
         ...
         if ($ContinueProcess)
         {
             $SuspensionID = dbAddSuspension($DbCon, $ChildID, $StartDate, $EndDate, $Reason);

             if ($SuspensionID != 0)
             {
                 // Log event
                 logEvent($DbCon, EVT_FAMILY, EVT_SERV_SUSPENSION, EVT_ACT_ADD, $_SESSION['SupportMemberID'], $SuspensionID);

                 // The suspension for the child is added
                 $ConfirmationCaption = $LANG_CONFIRMATION;
                 $ConfirmationSentence = $LANG_CONFIRM_CHILD_SUSPENSION_ADDED;
                 $ConfirmationStyle = "ConfirmationMsg";
                 $UrlParameters = "Cr=".md5($SuspensionID)."&Id=$SuspensionID"; // For the redirection
             }
             else
             {
                 // The suspension of the child can't be added
                 $ConfirmationCaption = $LANG_ERROR;
                 $ConfirmationSentence = $LANG_ERROR_ADD_CHILD_SUSPENSION;
 ...
 {
     // The supporter doesn't come from the AddChildSuspension.php page
     $ConfirmationCaption = $LANG_ERROR;
