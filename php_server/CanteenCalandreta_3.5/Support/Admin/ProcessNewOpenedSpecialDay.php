...
/**
 * Support module : process the creation of a new opened special day. The supporter must be logged.
 *
 ...
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
     {
         ...
         if ($ContinueProcess)
         {
             $OpenedSpecialDayID = dbAddOpenedSpecialDay($DbCon, $sDate, $Description);

             if ($OpenedSpecialDayID != 0)
             {
                 // The opened special day is added
                 $ConfirmationCaption = $LANG_CONFIRMATION;
                 $ConfirmationSentence = $LANG_CONFIRM_RECORD_ADDED;
                 $ConfirmationStyle = "ConfirmationMsg";
                 $UrlParameters = "UpdateOpenedSpecialDay.php?Cr=".md5($OpenedSpecialDayID)."&Id=$OpenedSpecialDayID"; // For the redirection
             }
             else
             {
                 // The opened special day can't be added
                 $ConfirmationCaption = $LANG_ERROR;
                 $ConfirmationSentence = $LANG_ERROR_ADD_RECORD;
                 $ConfirmationStyle = "ErrorMsg";
                 $UrlParameters = "AddOpenedSpecialDay.php?$QUERY_STRING"; // For the redirection
             }
         }
         else
         {
 ...
 {
     // The supporter doesn't come from the AddOpenedSpecialDay.php page
     $ConfirmationCaption = $LANG_ERROR;
