...
/**
 * Support module : reactivate a support member profil (if allowed by user rights). The supporter must be logged to
 ...
 */
 ...
 if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
 {
     ...
     if (($Id != '') && (md5($Id) == $CryptedID))
     {
         ...
         // We reactivate the selected support member
         if ($bCanDelete)
         {
             ...
             $id = dbUpdateSupportMember($DbCon, $SupportMemberID, $RecordSupportMember['SupportMemberLastname'],
                                         ...
                                         1, $RecordSupportMember['FamilyID']);
             if ($id != 0)
             {
                 // Log event
                 ...
                 $ConfirmationSentence = $LANG_CONFIRM_ACTIVATE_PROFIL;
             ...
             else
             {
                 ...
                 $ConfirmationSentence = $LANG_ERROR_ACTIVATE_PROFIL;
         ...
         {
             // Error : the user isn't allowed to reactivate a support member profil (because of wrong rights)
             $ConfirmationCaption = $LANG_ERROR;
