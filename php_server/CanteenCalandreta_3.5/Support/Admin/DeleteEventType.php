...
/**
 * Support module : delete an event type. The supporter must be logged to delete the event type.
 *
 ...
 */
 ...
 if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
 {
     ...
     if (($Id != '') && (md5($Id) == $CryptedID))
     {
         ...
         {
             // We delete the selected event type
             if (dbDeleteEventType($DbCon, $Id))
             {
                 ...
                 $UrlParameters = "EventTypesList.php"; // For the redirection
             }
             else
             {
                 // ERROR : the event type isn't deleted
         ...
         {
             // ERROR : the event type ID is wrong
             $ConfirmationCaption = $LANG_ERROR;
 ...
 {
     // Redirection to the event types list
     initGraphicInterface(
                          $LANG_INTRANET_NAME,
                          array(
                                '../../GUI/Styles/styles.css' => 'screen',
 ...
 {
     // Error because the ID of the event type ID and the crypted ID don't match
