...
/**
 * Support module : allow a supporter to create a new meeting room registration. The supporter must be logged
 ...
 */
 ...
 displayStyledLinkText($LANG_GO_TO_CONTENT, '#MeetingRoomRegistrationDetails', 'Accessibility');
 ...
 displayTitlePage($LANG_SUPPORT_CREATE_MEETING_ROOM_REGISTRATION_PAGE_TITLE, 2);

 // the ID and the md5 crypted ID must be equal ; if the ID is empty, the meeting room regisration will be a new meeting room registration
 if (md5($Id) == $CryptedID)
 {
     displayDetailsMeetingRoomRegistrationForm($DbCon, $Id, "ProcessNewMeetingRoomRegistration.php", $CONF_ACCESS_APPL_PAGES[FCT_MEETING], array());
 }
 else
 {
