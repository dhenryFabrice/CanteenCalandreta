...
/**
 * Support module : allow a supporter to create a new donation. The supporter must be logged
 ...
 */
 ...
 displayStyledLinkText($LANG_GO_TO_CONTENT, '#DonationDetails', 'Accessibility');
 ...
 displayTitlePage($LANG_SUPPORT_CREATE_DONATION_PAGE_TITLE, 2);

 // the ID and the md5 crypted ID must be equal ; if the ID is empty, the donation will be a new donation
 if (md5($Id) == $CryptedID)
 {
     displayDetailsDonationForm($DbCon, $Id, "ProcessNewDonation.php", $CONF_ACCESS_APPL_PAGES[FCT_DONATION]);
 }
 else
 {
