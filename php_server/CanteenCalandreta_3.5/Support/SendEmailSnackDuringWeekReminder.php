...
/**
 * Support module : Send an e-mail to concerned families during the week to remind
 * to bring the snack
 *
 ...
 */
 ...
 $NotificationType = 'RemindSnackDuringWeek';
 ...
 // We get the families who bring the snack during the current week
 $CurrentDate = date('Y-m-d');
 $CurrentSnackDate = $CurrentDate;
 if (date('w', strtotime($CurrentDate)) <> 1)
 {
     // We get the last monday if the current date isn't a monday
     $CurrentSnackDate = date('Y-m-d', strtotime("last monday", strtotime($CurrentDate)));
 }

 $ArraySnackRegistrations = getSnackRegistrations($DbCon, $CurrentSnackDate, $CurrentSnackDate, 'SnackRegistrationClass, SnackRegistrationDate',
    ...
    )
 {
     foreach($ArraySnackRegistrations['SnackRegistrationID'] as $sr => $SnackRegistrationID)
     {
         ...
         echo "Notification du goûter envoyée à la famille ".$ArraySnackRegistrations['FamilyLastname'][$sr]
             ...
             .", pour la classe de ".$CONF_CLASSROOMS[$SchoolYear][$ArraySnackRegistrations['SnackRegistrationClass'][$sr]].".<br />\n";

         $EmailSubject = $LANG_REMIND_SNACK_DURING_WEEK_EMAIL_SUBJECT;
         ...
         $ReplaceInTemplate = array(
                                    ...
                                    array(
                                          ...
                                          date($GLOBALS["CONF_DATE_DISPLAY_FORMAT"],
                                               strtotime($ArraySnackRegistrations['SnackRegistrationDate'][$sr]))
                                         )
                                   );
