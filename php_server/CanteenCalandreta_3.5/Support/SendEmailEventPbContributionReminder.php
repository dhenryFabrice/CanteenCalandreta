...
/**
 * Support module : Send an e-mail to families who don't respect the number of contributions
 * to event for a school year
 *
 ...
 */
 ...
 $NotificationType = 'NoIndicoopFamiliesReminder';
 ...
 // We get events with not enough registrations and registrations still opened
 $ArrayEvents = dbSearchEvent($DbCon, array(
                                            "OpenedRegistrations" => TRUE,
                                            "PbNbRegistrations" => TRUE
                                           ), "EventStartDate", 1, 0);
 ...
 {
     $EmailSubject = $LANG_REMIND_PB_CONTRIBUTION_EVENT_EMAIL_SUBJECT;
     ...
     foreach($ArrayEvents['EventID'] as $e => $EventID)
     {
         ...
         $EventsListMailContent .= "<li><a href=\"$EventUrl\" title=\"$EventLinkTip\" target=\"_blank\">$EventLink</a> : "
                                   .date($GLOBALS["CONF_DATE_DISPLAY_FORMAT"], strtotime($ArrayEvents['EventStartDate'][$e]));

         if (!empty($ArrayEvents['EventStartTime'][$e]))
         {
             $EventsListMailContent .= " ".$ArrayEvents['EventStartTime'][$e];
         }

         $EventsListMailContent .= ", $TownName ($TownCode)</li>";
     }
     ...
     {
         $ArrayRecipients = getEmailRecipients($DbCon, NULL, $RecipientType);
