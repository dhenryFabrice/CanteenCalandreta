...
/**
 * PHP plugin autocompletion module : add the autocompletion function for the sLastname field of the
     ...
     if (isset($CONF_COOP_WORKGROUP_REGISTRATION_VIEWS_RESTRICTIONS[$_SESSION['SupportMemberStateID']]))
     {
         switch($CONF_COOP_WORKGROUP_REGISTRATION_VIEWS_RESTRICTIONS[$_SESSION['SupportMemberStateID']])
         {
             case WORKGROUP_REGISTRATION_VIEWS_RESTRICTION_ALL:
                 // We get all lastname of activated families
                 $DbResult = $DbCon->query("SELECT f.FamilyLastname, s.SupportMemberLastname, wgr.WorkGroupRegistrationLastname
                                            ...
                                            AND ((FamilyLastname LIKE \"$sEnteredValue\") OR (SupportMemberLastname LIKE \"$sEnteredValue\")
                                            OR (WorkGroupRegistrationLastname LIKE \"$sEnteredValue\"))");

                 break;

             default:
                 // We filter only on lastname of the family
                 $DbResult = $DbCon->query("SELECT f.FamilyLastname, s.SupportMemberLastname, wgr.WorkGroupRegistrationLastname
                                            FROM Families f INNER JOIN SupportMembers s ON (f.FamilyID = s.FamilyID)
                                            LEFT JOIN WorkGroupRegistrations wgr ON (wgr.SupportMemberID = s.SupportMemberID)
         ...
         if (!DB::isError($DbResult))
         {
             ...
             if ($DbResult->numRows() > 0)
             {
                 ...
                 {
                     if (stripos(strToLower($Record['FamilyLastname']), $sCompareValue) !== FALSE)
                     {
                         $ArrayFoundValues[] = $Record['FamilyLastname'];
                     }

                     if (stripos(strToLower($Record['SupportMemberLastname']), $sCompareValue) !== FALSE)
                     {
                         $ArrayFoundValues[] = $Record['SupportMemberLastname'];
                     }

                     if ((!empty($Record['WorkGroupRegistrationLastname']))
                         && (stripos(strToLower($Record['WorkGroupRegistrationLastname']), $sCompareValue) !== FALSE))
                     {
                         $ArrayFoundValues[] = $Record['WorkGroupRegistrationLastname'];
                     }
                 }
             }
         }

         // Keep only one same lastname
