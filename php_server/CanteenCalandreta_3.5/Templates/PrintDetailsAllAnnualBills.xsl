...
<xsl:template match="/">
  ...
  <body>
    <div id="page">
      <xsl:apply-templates select="document/listannualbills" />
...
<xsl:template match="listannualbills">
  <xsl:for-each select="annualbill">
    <xsl:choose>
      <xsl:when test="position() = 1">
        <xsl:text disable-output-escaping="yes">&lt;div class="FamilyAnnualBill"&gt;</xsl:text>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text disable-output-escaping="yes">&lt;div class="FamilyAnnualBill" style="page-break-before: always;"&gt;</xsl:text>
      </xsl:otherwise>
    </xsl:choose>

      <h2>Famille <xsl:value-of select="familylastname" disable-output-escaping="yes" /></h2>
      <xsl:apply-templates select="billslist" />
...
<xsl:template match="billslist">
  <table cellspacing="0">
    <tr>
      <th colspan="7" class="Year"><xsl:value-of select="../../../year" /></th>
    </tr>
    <tr> 
    ...
    <tr>
      <td><xsl:value-of select="$balance" /></td>
      <td class="Balance">
        <xsl:value-of select="../familybalance" />
        <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
        <xsl:value-of select="$payment-unit" />
      </td>
      <td><xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text></td>
