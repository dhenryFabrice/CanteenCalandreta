...
/**
 * Support module : delete a support member state. The supporter must be logged to delete
 * the support member state.
 *
 ...
 */
 ...
 if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
 {
     ...
     if (($Id != '') && (md5($Id) == $CryptedID))
     {
         ...
         {
             // We delete the selected support member state
             if (dbDeleteSupportMemberState($DbCon, $Id))
             {
                 // The support member state is deleted
                 $ConfirmationCaption = $LANG_CONFIRMATION;
                 $ConfirmationSentence = $LANG_CONFIRM_RECORD_DELETED;
                 $ConfirmationStyle = "ConfirmationMsg";

                 $UrlParameters = "SupportMembersStatesList.php"; // For the redirection
             }
             else
             {
                 // ERROR : the support member state isn't deleted
         ...
         {
             // ERROR : the support member state ID is wrong
             $ConfirmationCaption = $LANG_ERROR;
 ...
 {
     // Redirection to the support member states list
     initGraphicInterface(
                          $LANG_INTRANET_NAME,
                          array(
                                '../../GUI/Styles/styles.css' => 'screen',
 ...
 {
     // Error because the ID of the support member state ID and the crypted ID don't match
