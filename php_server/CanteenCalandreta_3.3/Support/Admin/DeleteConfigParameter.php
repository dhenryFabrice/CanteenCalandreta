...
/**
 * Support module : delete a config parameter. The supporter must be logged to delete
 ...
 */
 ...
 // To take into account the crypted and no-crypted config parameter ID
 // Crypted ID
 if (!empty($_GET["Cr"]))
 {
 ...
 if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
 {
     ...
     if (($Id != '') && (md5($Id) == $CryptedID))
     {
         ...
         {
             // We delete the selected config parameter
             if (dbDeleteConfigParameter($DbCon, $Id))
             {
                 // The config parameter is deleted
                 $ConfirmationCaption = $LANG_CONFIRMATION;
                 $ConfirmationSentence = $LANG_CONFIRM_RECORD_DELETED;
                 $ConfirmationStyle = "ConfirmationMsg";

                 $UrlParameters = "ConfigParametersList.php"; // For the redirection
             }
             else
             {
                 // ERROR : the config parameter isn't deleted
         ...
         {
             // ERROR : the config parameter ID is wrong
             $ConfirmationCaption = $LANG_ERROR;
 ...
 {
     // Redirection to the config parameters list
     initGraphicInterface(
                          $LANG_INTRANET_NAME,
                          array(
                                '../../GUI/Styles/styles.css' => 'screen',
 ...
 {
     // Error because the ID of the config parameter ID and the crypted ID don't match
