 ...
 */
 ...
 // To take into account the page of the holidays to display
 if (!empty($_GET["Pg"]))
 {
 ...
 // To take into account the order by field to sort the table of the holidays
 ...
 if ((!empty($_POST["bSubmit"])) || (array_key_exists("hidOrderByField", $_POST)) || ($LaunchSearch))
 {
     ...
     {
         $HolidayDescription = trim(strip_tags(${$ParamsPOST_GET}["sDescription"]));
         if (!empty($HolidayDescription))
         {
             $TabParams["HolidayDescription"] = $HolidayDescription;
         }
     }

     // <<< HolidayStartDate field >>>
     if (array_key_exists("startDate", ${$ParamsPOST_GET}))
     {
         ...
         if (!empty($StartDate))
         {
             $TabParams["HolidayStartDate"] = array($StartDate, $CONF_LOGICAL_OPERATORS[${$ParamsPOST_GET}["lOperatorStartDate"]]);
         }
     }

     // <<< HolidayEndDate field >>>
     if (array_key_exists("endDate", ${$ParamsPOST_GET}))
     {
         ...
         if (!empty($EndDate))
         {
             $TabParams["HolidayEndDate"] = array($EndDate, $CONF_LOGICAL_OPERATORS[${$ParamsPOST_GET}["lOperatorEndDate"]]);
         }
     }
 ...
 {
     // First display : by default, holidays of the current school year
     $SchoolYear = getSchoolYear(date('Y-m-d'));
     $CurrentSchoolStartDate = getSchoolYearStartDate($SchoolYear);

     $TabParams["HolidayStartDate"] = array($CurrentSchoolStartDate, ">=");
     $TabParams["All"] = TRUE;
 }
 //################################ END FORM PROCESSING ##########################
 ...
 if ($HidOnPrint == 0)
 {
     ...
     displayStyledLinkText($LANG_GO_TO_CONTENT, '#HolidaysList', 'Accessibility');
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         displaySupportMemberContextualMenu("admin", 1, Admin_HolidaysList);
     ...
     displayTitlePage($LANG_SUPPORT_ADMIN_HOLIDAYS_LIST_PAGE_TITLE, 2);

     openParagraph();
     echo $LANG_SUPPORT_ADMIN_HOLIDAYS_LIST_PAGE_INTRODUCTION;
     closeParagraph();

     // Display a button to add a new holiday
     openParagraph();
     echo generateStyledPictureHyperlink($GLOBALS["CONF_ADD_ICON"], "../Admin/AddHoliday.php?Cr=".md5('')."&amp;Id=",
                                         "$LANG_SUPPORT_ADMIN_HOLIDAYS_LIST_PAGE_ADD_HOLYDAY.", 'Affectation', '_blank')
          ." $LANG_SUPPORT_ADMIN_HOLIDAYS_LIST_PAGE_ADD_HOLYDAY.";
     closeParagraph();

     displaySearchHolidaysForm($DbCon, $TabParams, "HolidaysList.php", $Page, "SortFct", $OrderBy,
                               "UpdateHoliday.php", $CONF_ACCESS_APPL_PAGES[FCT_ADMIN]);
     ...
     if ($HidOnExport == 1)
     {
         exportSearchHolidaysForm($DbCon, $TabParams, ${$ParamsPOST_GET}["hidExportFilename"]);
 ...
 {
     // Print the web page
     printSearchHolidaysForm($DbCon, $TabParams);
