...
/**
 * Support module : process the update of an event type. The supporter must be logged.
 *
 ...
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
     {
         ...
         if (isExistingEventType($DbCon, $Id))
         {
         ...
         if ($ContinueProcess)
         {
             $EventTypeID = dbUpdateEventType($DbCon, $EventTypeID, $EventTypeName, $EventTypeCategory);
             if ($EventTypeID != 0)
             {
                 ...
                 $UrlParameters = "Cr=".md5($EventTypeID)."&Id=$EventTypeID"; // For the redirection
             }
             else
             {
                 // The event type can't be updated
 ...
 {
     // The supporter doesn't come from the UpdateEventType.php page
     $ConfirmationCaption = $LANG_ERROR;
 ...
 initGraphicInterface(
                      ...
                      'WhitePage',
                      "Redirection('".$CONF_ROOT_DIRECTORY."Support/Admin/UpdateEventType.php?$UrlParameters', $CONF_TIME_LAG)"
                     );
