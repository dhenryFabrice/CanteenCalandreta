...
/**
 * Support module : delete an opened special day. The supporter must be logged to delete the opened special day.
 *
 ...
 */
 ...
 if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
 {
     ...
     if (($Id != '') && (md5($Id) == $CryptedID))
     {
         ...
         {
             // We delete the selected opened special day
             if (dbDeleteOpenedSpecialDay($DbCon, $Id))
             {
                 // The opened special day is deleted
                 $ConfirmationCaption = $LANG_CONFIRMATION;
                 $ConfirmationSentence = $LANG_CONFIRM_RECORD_DELETED;
                 $ConfirmationStyle = "ConfirmationMsg";

                 $UrlParameters = "OpenedSpecialDaysList.php"; // For the redirection
             }
             else
             {
                 // ERROR : the opened special day isn't deleted
         ...
         {
             // ERROR : the opened special day ID is wrong
             $ConfirmationCaption = $LANG_ERROR;
 ...
 {
     // Redirection to the opened special days list
     initGraphicInterface(
                          $LANG_INTRANET_NAME,
                          array(
                                '../../GUI/Styles/styles.css' => 'screen',
 ...
 {
     // Error because the ID of the opened special day ID and the crypted ID don't match
