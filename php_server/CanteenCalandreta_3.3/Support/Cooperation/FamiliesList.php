...
/**
 * Support module : list some families with their cooperation to events
 *
 ...
 */
 ...
 if ((!empty($_POST["bSubmit"])) || (array_key_exists("hidOrderByField", $_POST)) || ($LaunchSearch))
 {
     ...
     // Only families with no pb of contributions to events
     $bNoPbCooperationChecked = FALSE;
     if (array_key_exists("chkEventContributions", ${$ParamsPOST_GET}))
     {
         $TabParams["FamilyCoopContribution"] = $SchoolYear;
         $bNoPbCooperationChecked = TRUE;
     }

     // Only families with pb of contributions to events
     $bPbCooperationChecked = FALSE;
     if (array_key_exists("chkPbEventContributions", ${$ParamsPOST_GET}))
     {
         $TabParams["FamilyPbCoopContribution"] = $SchoolYear;
         $bPbCooperationChecked = TRUE;
     }

     if (($bNoPbCooperationChecked) && ($bPbCooperationChecked))
     {
         // Both are checked : not possible, we cancel both options
         unset($TabParams["FamilyCoopContribution"], $TabParams["FamilyPbCoopContribution"]);
     }

     // Only activated families
 ...
 if ($HidOnPrint == 0)
 {
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         displaySupportMemberContextualMenu("cooperation", 1, Coop_FamiliesList);
     ...
     displayTitlePage($LANG_SUPPORT_FAMILIES_EVENT_CONTRIBUTIONS_LIST_PAGE_TITLE, 2);

     openParagraph();
     echo $LANG_SUPPORT_FAMILIES_EVENT_CONTRIBUTIONS_LIST_PAGE_INTRODUCTION;
     closeParagraph();
     ...
     switch($_SESSION['SupportMemberStateID'])
     {
         ...
         default:
             $ArrayDisplayedFormFields = array(
                                               "lSchoolYear" => true,
                                               "sLastname" => false,
                                               "chkEventContributions" => false,
                                               "chkPbEventContributions" => false
                                              );
             break;
     }

     displaySearchFamilyEventContributionForm($DbCon, $TabParams, "FamiliesList.php", $Page, "SortFct", $OrderBy, "../Canteen/UpdateFamily.php",
