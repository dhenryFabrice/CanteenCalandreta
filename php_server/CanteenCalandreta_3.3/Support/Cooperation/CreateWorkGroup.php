...
/**
 * Support module : allow a supporter to create a new workgroup. The supporter must be logged
 ...
 */
 ...
 displayStyledLinkText($LANG_GO_TO_CONTENT, '#WorkGroupDetails', 'Accessibility');
 ...
 displayTitlePage($LANG_SUPPORT_CREATE_WORKGROUP_PAGE_TITLE, 2);

 // the ID and the md5 crypted ID must be equal ; if the ID is empty, the workgroup will be a new workgroup
 if (md5($Id) == $CryptedID)
 {
     displayDetailsWorkGroupForm($DbCon, $Id, "ProcessNewWorkGroup.php", array($CONF_ACCESS_APPL_PAGES[FCT_WORKGROUP],
                                 $CONF_ACCESS_APPL_PAGES[FCT_WORKGROUP_REGISTRATION]));
 }
 else
 {
     openFrame($LANG_ERROR);
     displayStyledText($LANG_ERROR_NOT_VIEW_WORKGROUP, "ErrorMsg");
     closeFrame();
 }
