 ...
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     ...
     $MinEditDateStamp = strtotime(date('Y-m-d', strtotime($GLOBALS['CONF_NURSERY_UPDATE_DELAY_PLANNING_REGISTRATION']." days ago")));
     ...
     if (isset($_POST['chkNurseryRegitrationAM']))
     {
         ...
         {
             $ArrayPOST[$ChkValue] = array('AM' => 1, 'PM' => 0);
         }
     }
     ...
     if (isset($_POST['chkNurseryRegitrationPM']))
     {
         foreach($_POST['chkNurseryRegitrationPM'] as $p => $ChkValue)
         {
             ...
             {
                 $ArrayPOST[$ChkValue] = array('AM' => 0, 'PM' => 1);
             }
         }
     }
     ...
     if (!empty($ArrayPOST))
     {
         ...
         foreach($ArrayPOST as $sValue => $AMPM)
         {
             ...
             if (count($ArrayValues) == 4)
             {
                 ...
                 if (($bCanEdit) && (empty($NurseryRegistrationID)))
                 {
                     ...
                     $NurseryRegistrationID = dbAddNurseryRegistration($DbCon, $TodayDate, $ForDate, $ChildID,
                                                                       $_SESSION['SupportMemberID'], $AMPM['AM'], $AMPM['PM'],
                 ...
                 else
                 {
                     ...
                     if (($bCanEdit) && (isExistingNurseryRegistration($DbCon, $NurseryRegistrationID)))
                     {
                         if (($AMPM['AM'] == 1) || ($AMPM['PM'] == 1))
                         {
                             ...
                             if (($ArrayChildren['NurseryRegistrations'][$ArrayPosChildren[$ChildID]][$ForDate]['NurseryRegistrationForAM'] != $AMPM['AM'])
                                 || ($ArrayChildren['NurseryRegistrations'][$ArrayPosChildren[$ChildID]][$ForDate]['NurseryRegistrationForPM'] != $AMPM['PM'])
                                )
                             {
                                 // Yes, it's an update
                                 $NurseryRegistrationID = dbUpdateNurseryRegistration($DbCon, $NurseryRegistrationID, NULL, $ForDate,
                                                                                      $ChildID, $_SESSION['SupportMemberID'], $AMPM['AM'],
                                                                                      $AMPM['PM'], $ArrayChildren['ChildGrade'][$ArrayPosChildren[$ChildID]],
                                                                                      $ArrayChildren['ChildClass'][$ArrayPosChildren[$ChildID]],
                                                                                      $AdminDate);
