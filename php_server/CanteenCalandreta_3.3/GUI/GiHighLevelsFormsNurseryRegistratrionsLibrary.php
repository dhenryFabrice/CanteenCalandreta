     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         if (in_array($cUserAccess, array(FCT_ACT_CREATE, FCT_ACT_UPDATE, FCT_ACT_READ_ONLY, FCT_ACT_PARTIAL_READ_ONLY)))
         {
             ...
             foreach($Days as $i => $CurrentDay)
             {
                 ...
                 {
                     // Nursery opened
                     echo "<th class=\"PlanningCaptions\" colspan=\"2\">$Prefix ".sprintf("%02u", $NumCurrentDay)."</th>";
                 }
                 else
                 {
                     // Nursery not opened
                     echo "<th class=\"PlanningCaptionsHoliday\" colspan=\"2\">$Prefix ".sprintf("%02u", $NumCurrentDay)."</th>";
                 }
             }
             ...
             $ArrayTotalsDaysOfMonth = array(
                                             array(),
                                             array()
                                            );  // To store nb nursery registrations for each day (AM is the first array and PM, the second array)
             ...
             foreach($ArrayChildren["ChildID"] as $i => $CurrentChildID)
             {
                 if ($CurrentClass != $ArrayChildren["ChildClass"][$i])
                 {
                     ...
                     echo "<tr>\n\t<td class=\"PlanningSplit\" colspan=\"".(2 * $NbDays + 2)."\"><strong>"
                     ...
                     foreach($Days as $j => $CurrentDay)
                     {
                         ...
                         if (!isset($ArrayTotalsDaysOfMonth[1][$CurrentDayDate]))
                         {
                             // To count the nb of nursery registrations for this day, for PM
                             $ArrayTotalsDaysOfMonth[1][$CurrentDayDate] = 0;
                         }
                             ...
                             && (!empty($ArrayNurseryRegistrationsOfChild['NurseryRegistrationID'][$iPosCurrentDayDate])))
                         {
                             ...
                             if ($bNurseryOpenedPM)
                             {
                                 ...
                                 else
                                 {
                                     ...
                                     $iNbNurseryRegistrations++;
                                     $ArrayTotalsDaysOfMonth[1][$CurrentDayDate]++;
                                 }
             ...
             echo "<tr>\n\t<td class=\"PlanningSplitMoreMeals\" colspan=\"".(2 * $NbDays + 2)."\">&nbsp;</td>\n</tr>\n";
             ...
             foreach($Days as $j => $CurrentDay)
             {
                 ...
                 if ((in_array($NumCurrentDay, $OpenedDays[$Offset])) || ($iPosOpenedSpecialDay !== FALSE))
                 {
                     ...
                     if (($GLOBALS['CONF_NURSERY_OPENED_WEEK_DAYS'][$iNumWeekDay - 1][1]) || ($iPosOpenedSpecialDay !== FALSE))
                     {
                         ...
                         if (isset($ArrayTotalsDaysOfMonth[1][$CurrentDayDate]))
                         {
                             $QuantityPM = $ArrayTotalsDaysOfMonth[1][$CurrentDayDate];
                         }
                     }
                     else
                     {
             ...
             echo "<td class=\"PlanningTotalMonth\">".(array_sum($ArrayTotalsDaysOfMonth[0]) + array_sum($ArrayTotalsDaysOfMonth[1]))."</td>\n</tr>\n";
                 ...
                 foreach($ArrayNurseryRegistrations['NurseryRegistrationID'] as $cr => $CurrentID)
                 {
                     if ((is_null($PreviousClassroom)) || ($PreviousClassroom != $ArrayNurseryRegistrations['ChildClass'][$cr]))
                     {
                         ...
                         echo "<tr>\n\t<th>".$GLOBALS['LANG_FAMILY_LASTNAME']."</th><th>".$GLOBALS['LANG_CHILD_FIRSTNAME']
                              ."</th><th>".$GLOBALS['LANG_CHILD_GRADE']."</th><th>".$GLOBALS['LANG_AM']."</th><th>"
                              .$GLOBALS['LANG_PM']."</th>\n</tr>\n";
