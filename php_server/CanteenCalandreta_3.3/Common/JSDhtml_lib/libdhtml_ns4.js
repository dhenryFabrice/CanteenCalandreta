...
/*
DHTML Librairie NS4.x (Version 2.3 - 03/12/2000)
Copyright E.D Prod.
...
function CreerObj(nom,px,py,tx,ty,visible,zindex,contenu,special,dragdrop){if (visible==1) {visi="show"} else {visi="hidden"};chaine='<layer width="'+tx+'" height="'+ty+'" top="'+py+'" left="'+px+'" z-index="'+zindex+'" visibility="'+visi+'" name="'+nom+'" '+special+'>'+contenu+'</layer>';document.write(chaine);Nobj[PtObj]=nom;SobjX[PtObj]=tx;SobjY[PtObj]=ty;Dobj[PtObj]=0;DDobj[PtObj]=dragdrop;PtObj+=1;}
function CreerZone(nom,x1,y1,x2,y2){document.layers[nom].clip.left = x1;document.layers[nom].clip.top = y1;document.layers[nom].clip.right = x2;document.layers[nom].clip.bottom = y2;}
function CacherObj(nom){document.layers[nom].visibility = "hidden";}
function ChangeIndex(nom,valeur) {document.layers[nom].zIndex=valeur;}
function VoirObj(nom){document.layers[nom].visibility = "visible";}
function ModifierObj(nom,contenu){document.layers[nom].document.writeln(contenu);document.layers[nom].document.close();}
function PlacerObj(nom,px,py) {if (px!=-1){document.layers[nom].left = px;}if (py!=-1){document.layers[nom].top = py;}}
function ObjX(nom){var chaine=document.layers[nom].left;var value=parseInt(chaine);return value;}
function ObjY(nom){var chaine=document.layers[nom].top;var value=parseInt(chaine);return value;}
document.captureEvents(Event.MOUSEMOVE|Event.MOUSEDOWN|Event.MOUSEUP);
function mouvement(evnt) {MXP = evnt.pageX;MYP = evnt.pageY;for ( j = 0 ; j < PtObj ; j++ ) {if (Dobj[j]==1){PlacerObj(Nobj[j],MXP-decaX,MYP-decaY);return false;}}}
function Clique(evnt) {MXP = evnt.pageX;MYP = evnt.pageY;
for ( j = 0 ; j < PtObj ; j++ ) {if (DDobj[j]==1){if ((MXP>=ObjX(Nobj[j])) && (MXP<=ObjX(Nobj[j])+SobjX[j]) && (MYP>=ObjY(Nobj[j])) && (MYP<=ObjY(Nobj[j])+SobjY[j])){Dobj[j]=1;decaX=MXP-ObjX(Nobj[j]);decaY=MYP-ObjY(Nobj[j]);return false;}else {Dobj[j]=0;}}}}
...
function CreerStaticObj(nom,px,py,tx,ty,visible,zindex,contenu,special,dragdrop){if (visible==1) {visi="show"} else {visi="hidden"};chaine='<layer width="'+tx+'" height="'+ty+'" top="'+py+'" left="'+px+'" z-index="'+zindex+'" visibility="'+visi+'" name="'+nom+'" '+special+'>'+contenu+'</layer>';document.write(chaine);Nobj[PtObj]=nom;SobjX[PtObj]=tx;SobjY[PtObj]=ty;Dobj[PtObj]=0;DDobj[PtObj]=dragdrop;PtObj+=1;}
function CacherStaticObj(nom){document.layers[nom].visibility = "hidden";}
function VoirStaticObj(nom){document.layers[nom].visibility = "visible";}
function VisibiliteObj(nom){if (ddocument.layers[nom].visibility == "hidden") {return false} else {return true};}
